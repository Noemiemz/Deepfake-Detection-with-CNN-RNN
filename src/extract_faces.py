...
def extract_n_first_faces_from_videos(
    input_folder: str,
    output_images_dir: str,
    n: int = 5,
    images_per_second: int = 0,
):
    """
    Extracts the first n faces from all .mp4 files in the input folder and saves face crops.

    Args:
        input_folder: Path to folder containing .mp4 files.
        output_images_dir: Path to save extracted face images.
        n: Number of faces to extract per video (default: 5).
        images_per_second: Number of frames to extract per second (default: 60).
    """
    print(f"Downloading YOLO face detection model...")
    model_path = hf_hub_download(repo_id="arnabdhar/YOLOv8-Face-Detection", filename="model.pt")
    print(f"Model downloaded to {model_path}")
    ...
    for video_file in tqdm(list(Path(input_folder).rglob("*.mp4")), desc="Videos"):
        cap = cv2.VideoCapture(str(video_file))
        fps = cap.get(cv2.CAP_PROP_FPS)
        frame_interval = int(fps // images_per_second) if images_per_second > 0 else fps
        frame_id = 0
        saved_frame_id = 0
        imgs_of_vid_dir = os.path.join(output_images_dir, video_file.stem)
        os.makedirs(imgs_of_vid_dir, exist_ok=True)

        while saved_frame_id < n:
            ret, frame = cap.read()
            if not ret:
                break
            ...
            frame_id += 1
        cap.release()
    print(f"Face extraction completed for first {n} faces per video!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_first_faces", type=int, default=None, help="If set, extract only the first n faces from each video.")
    
    args = parser.parse_args()
    
    if args.n_first_faces is not None:
        extract_n_first_faces_from_videos(
            input_folder=args.input_folder,
            output_images_dir=args.output_images_dir,
            n=args.n_first_faces,
            images_per_second=args.frames_per_second,
        )
    else:
        extract_faces_from_videos(
            ...
            nb_frames_per_time_window=args.nb_frames_per_time_window,
        )
